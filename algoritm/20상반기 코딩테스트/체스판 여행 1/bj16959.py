...
#             N=n
#             if A[Y][X]==n+1:N+=1
#             if V[Y][X][N][s]==-1 or t<V[Y][X][N][s]:
#                 V[Y][X][N][s]=t
#                 Q.append((Y,X,N,s,t))
#             Y,X=Y+dy[s][d],X+dx[s][d]
# N=int(input())
# A=[[int(x)for x in input().split()]for y in range(N)]
# V=[[[[-1]*3 for _ in range(N*N+1)]for __ in range(N)]for ___ in range(N)]
# Q=deque()
# for i in range(N):
#     for j in range(N):
#         if A[i][j]==1:
#             for x in range(3):Q.append((i,j,1,x,0));V[i][j][1][x]=0
#         elif A[i][j]==N*N:Y,X=i,j
# R=10**9
# while Q:
#     y,x,now,s,t=Q.popleft()
#     if now==N*N:R=min(R,t);continue
#     for S in range(3):
#         Now=now
#         T=t+1 if S==s else t+2
#         if S==0:Knight(y,x,now,T)
#         else:BL(y,x,now,T,S)
# print(R)
